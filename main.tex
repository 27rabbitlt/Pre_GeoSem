...
\begin{frame}
	...
	\begin{center}
		\begin{tikzpicture}[tdplot_main_coords, scale=2]
			...
			\coordinate (P) at ($2*(A)-(B)$);

			\draw[fill=red!50] (O) -- (A) -- (E) -- (D) -- cycle;
			\draw[fill=blue!50] (O) -- (A) -- (B) -- (C) -- cycle;
			\draw[fill=blue!50] (B) -- (C) -- (G) -- (F) -- cycle;
			% Draw front face
			...
			\draw[dashed, thick] (A) -- (P) node[left] {$p$};
	...
	\begin{tikzpicture}[scale=1]
		...
		% \draw[fill=gray!50, opacity=0.8] (0,-0.9) rectangle +(0.4,0.4);
